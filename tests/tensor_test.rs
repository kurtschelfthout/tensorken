...
fn reduce_ops<T: RealizedRawTensor<E = f32>>(t1: &Tensor<T>) {
    let r1 = t1.sum(&[0]);
...
fn movement_ops<T: RealizedRawTensor<E = f32>>(t1: &Tensor<T>) {
...
fn do_2x3_matmul_3x2<T: RealizedRawTensor<E = f32>>(t1: &Tensor<T>, t2: &Tensor<T>) {
    let r1 = t1.matmul(t2);
...
fn do_2x3x5_matmul_2x5x2<T: RealizedRawTensor<E = f32>>(t1: &Tensor<T>, t2: &Tensor<T>) {
    let r1 = t1.matmul(t2);
...
fn do_3x2x2x3_matmul_2x3x2<T: RealizedRawTensor<E = f32>>(t1: &Tensor<T>, t2: &Tensor<T>) {
    let r = t1.matmul(t2);
...
fn do_eye_test<T: RealizedRawTensor<E = f32>>(dim: usize) {
...
fn do_test_at<T: RealizedRawTensor<E = f32>>(t: &Tensor<T>) {
    let s = t.at(1);
    assert_eq!(s.shape(), &[4]);
