...
/// Forward AD implementation WITH tracing.
/// Not currently used or compiled (it's not in lib.rs), but a more or less drop-in replacement for ad_forward.
/// Follows the same pattern as ad_reverse, differences with ad_forward:
/// - a PushForward type that needs to be called
/// - more lifetime parameters to Forward because of the trace
/// Decided against this in the end, because although it has some pleasing symmetry,
/// the extra memory usage in forward-mode AD this entails is unexpected.
